...
//  } else {
//    p->sync_start_cb.Run(BTM_UNKNOWN_ADDR, 0, p->sid, 0, p->remote_bda, 0, 0);
//    memset(p, 0, sizeof(tBTM_BLE_PERIODIC_SYNC));
...
static void btm_ble_start_sync_timeout(void *data) {
  ...
  p->sync_start_cb.Run(HCI_ERR_CONN_FAILED_ESTABLISHMENT, 0, p->sid, 0, p->remote_bda, 0, 0);
  memset(p, 0, sizeof(tBTM_BLE_PERIODIC_SYNC));
  btm_sync_queue_advance();
}
...
void btm_ble_periodic_adv_sync_established(uint8_t *param, uint16_t param_len) {
  ...
  if (status == BTM_SUCCESS) {
    ...
    memset(ps, 0, sizeof(tBTM_BLE_PERIODIC_SYNC));
  }
  btm_sync_queue_advance();
}
