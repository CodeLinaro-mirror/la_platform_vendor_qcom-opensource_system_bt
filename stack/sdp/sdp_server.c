...
BOOLEAN sdp_dev_blacklisted_for_avrcp15 (BD_ADDR addr)
{
    ...
    if (interop_addr_match(INTEROP_ADV_AVRCP_VER_1_3, &remote_bdaddr)) {
        ...
        if (strlen((const char *)bdname.name) != 0 &&
            interop_name_match(INTEROP_ADV_AVRCP_VER_1_3, (const char *)bdname.name))
        {
                                      ...
                                      BD_ADDR remote_address)
{
        ...
        UUID_SERVCLASS_PBAP_PSE))
    {
        ...
        if (interop_addr_match(INTEROP_ADV_PBAP_VER_1_1, &remote_bdaddr) ||
           (strlen((const char *)bdname.name) != 0 &&
            interop_name_match(INTEROP_ADV_PBAP_VER_1_1,
            (const char *)bdname.name))) {
            SDP_TRACE_DEBUG("%s: device is blacklisted", __func__);
            return TRUE;
        }
    }
