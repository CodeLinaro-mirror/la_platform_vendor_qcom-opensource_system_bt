...
static bthh_vendor_callbacks_t* bt_hh_vendor_callbacks = NULL;
...
static void btif_hh_upstreams_evt(uint16_t event, char* p_param) {
  ...
  switch (event) {
    ...
    case BTA_HH_SEND_RAW_DATA_EVT:
       ...
       p_dev = btif_hh_find_connected_dev_by_handle(p_data->raw_data.dev_handle);
       HAL_CBACK(bt_hh_vendor_callbacks, raw_hid_data_cb,p_dev->bd_addr,
 ...
 *
 * Description      Closes the HH vendor interface
 *
 * Returns          bt_status_t
 *
 ******************************************************************************/
