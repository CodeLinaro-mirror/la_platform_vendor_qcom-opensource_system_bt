...
static const char* SBC_ENCODER_1_LIB_NAME = "libbt-sbc-encoder1.so";
...
static const char* get_lib_name_by_address(const RawAddress& peer_address) {
  ...
  if (address_libs_map.find(peer_address) != address_libs_map.end()) {
    LOG_DEBUG(LOG_TAG, "%s, address_libs_map is not empty", __func__);
    lib_name = address_libs_map.find(peer_address)->second;
    if (strcmp(lib_name, encoder_libs[0]) != 0 &&
          strcmp(lib_name, encoder_libs[1]) != 0) {
        LOG_ERROR(LOG_TAG, "%s: unknown lib name %s", __func__, lib_name);
        return nullptr;
    }
    LOG_WARN(LOG_TAG, "%s: lib %s has been used for current address", __func__, lib_name);
    return lib_name;
  }
  ...
  for (int i = 0; i < MAX_SBC_ENDOER_LIBS; i++) {
    ...
    for (auto it : address_libs_map) {
      ...
      if (strcmp(lib_name, encoder_libs[i]) == 0) {
          LOG_DEBUG(LOG_TAG, "%s: lib name: %s has been used", __func__, lib_name);
          used = true;
          break;
      }
    }
    if (!used) {
      ...
      LOG_DEBUG(LOG_TAG, "%s: find lib name: %s", __func__, lib_name);
      address_libs_map.emplace(peer_address, lib_name);
      return lib_name;
    }
  }
  LOG_ERROR(LOG_TAG, "%s: No lib found", __func__);
  return nullptr;
}
...
void A2dpSbcEncoder::encoder_init(tA2DP_ENCODER_INIT_PEER_PARAMS* p_peer_params,
  ...
  this->is_peer_edr = p_peer_params->is_peer_edr;
  this->peer_supports_3mbps = p_peer_params->peer_supports_3mbps;
  this->peer_mtu = p_peer_params->peer_mtu;
  this->timestamp = 0;
  ...
  LOG_DEBUG(LOG_TAG, "%s: Open sbc encoder library %s", __func__, lib_name);
  sbc_encoder_lib_handle = dlopen(lib_name, RTLD_NOW);
  if (sbc_encoder_lib_handle == NULL) {
    LOG_ERROR(LOG_TAG, "%s: cannot open sbc encoder library %s: %s", __func__,
              lib_name, dlerror());
    return;
  }

  mSbcUpSample = new A2DP_SBC_UPS_CB();
  a2dp_sbc_encoder_update(this->peer_mtu, a2dp_codec_config,
...
void A2dpSbcEncoder::a2dp_sbc_encoder_update(uint16_t peer_mtu,
                                    A2dpCodecConfig* a2dp_codec_config,
  ...
  *p_config_updated = false;
  ...
  if (sbcif == NULL) {
    LOG_ERROR(LOG_TAG,
              "%s: cannot find symbol '%s' in the encoder library. Error: %s",
  ...
  if (mtu_size < peer_mtu) {
    this->TxAaMtuSize = mtu_size;
  } else {
    this->TxAaMtuSize = peer_mtu;
  }
  ...
  LOG_DEBUG(LOG_TAG, "%s: MTU=%d, peer_mtu=%d min_bitpool=%d max_bitpool=%d",
            __func__, this->TxAaMtuSize, peer_mtu, min_bitpool,
            max_bitpool);
  LOG_DEBUG(LOG_TAG,
...
bool A2dpSbcEncoder::a2dp_sbc_read_feeding(uint32_t* bytes_read) {
  ...
  if (sbc_sampling == this->feeding_params.sample_rate) {
    LOG_VERBOSE(LOG_TAG, "%s: %s, sbc_sampling == sample_rate", __func__, get_peer_address().ToString().c_str());
    read_size =
        bytes_needed - this->feeding_state.aa_feed_residue;
    this->stats.media_read_total_expected_read_bytes += read_size;
    nb_byte_read = /*this->read_callback*/btif_a2dp_source_read_callback(
        get_peer_address(),
    ...
    *bytes_read = nb_byte_read;
    if (nb_byte_read != read_size) {
      LOG_WARN(LOG_TAG, "%s: %s, nb_byte_read %d != read_size %d", __func__,
    ...
    this->feeding_state.aa_feed_residue = 0;
    LOG_VERBOSE(LOG_TAG, "%s: %s, return true", __func__, get_peer_address().ToString().c_str());
    return true;
  }
  ...
  nb_byte_read =
      /*this->read_callback*/btif_a2dp_source_read_callback(get_peer_address(),
                                (uint8_t*)read_buffer, read_size);
  this->stats.media_read_total_actual_read_bytes += nb_byte_read;

  if (nb_byte_read < read_size) {
    if (nb_byte_read == 0) {
      LOG_WARN(LOG_TAG, "%s: %s, nb_byte_read is 0", __func__, get_peer_address().ToString().c_str());
      return false;
    }
  ...
  if (this->feeding_state.aa_feed_residue < bytes_needed) {
    LOG_WARN(LOG_TAG, "%s: %s, aa_feed_residue:%d, bytes_needed:%d", __func__,
...
void A2dpSbcEncoder::a2dp_sbc_encode_frames(uint8_t nb_frame) {
  ...
  if (sbcif == NULL) {
    LOG_ERROR(LOG_TAG,
              "%s: cannot find sysmbol '%s' in the encoder library, error: %s",
              __func__, SBC_ENCODE_INTERFACE_STRING, dlerror());
    dlclose(sbc_encoder_lib_handle);
    return ;
  }
  ...
  LOG_VERBOSE(LOG_TAG, "%s: peer_address %s, done", __func__,
              get_peer_address().ToString().c_str());
}

uint8_t A2dpSbcEncoder::calculate_max_frames_per_packet(void) {
  uint16_t effective_mtu_size = this->TxAaMtuSize;
  SBC_ENC_PARAMS* p_encoder_params = &this->sbc_encoder_params;
  uint16_t result = 0;
  uint32_t frame_len;
  ...
  if (this->is_peer_edr &&
    ...
    if (effective_mtu_size > MAX_2MBPS_AVDTP_MTU) {
      ...
      this->TxAaMtuSize = effective_mtu_size;
    }
  }
...
uint16_t A2dpSbcEncoder::a2dp_sbc_source_rate() {
...
void A2dpSbcEncoder::a2dp_sbc_get_num_frame_iteration(uint8_t* num_of_iterations,
  ...
  this->feeding_state.counter +=
      this->feeding_state.bytes_per_tick * us_this_tick /
      (A2DP_SBC_ENCODER_INTERVAL_MS * 1000);

  /* Calculate the number of frames pending for this->media tick */
  projected_nof =
      this->feeding_state.counter / pcm_bytes_per_frame;
  // Update the stats
  ...
  this->feeding_state.counter -= noi * nof * pcm_bytes_per_frame;
...
bool A2dpCodecConfigSbcSource::updateEncoderUserConfig(
  ...
  encoder->is_peer_edr = p_peer_params->is_peer_edr;
  encoder->peer_supports_3mbps = p_peer_params->peer_supports_3mbps;
  encoder->peer_mtu = p_peer_params->peer_mtu;
  encoder->timestamp = 0;

  if (encoder->peer_mtu == 0) {
    LOG_ERROR(LOG_TAG,
              "%s: Cannot update the codec encoder for %s: "
              "invalid peer MTU",
              __func__, name().c_str());
  ...
  encoder->a2dp_sbc_encoder_update(encoder->peer_mtu, this, p_restart_input,
