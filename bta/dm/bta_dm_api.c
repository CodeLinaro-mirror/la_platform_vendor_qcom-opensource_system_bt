                     ...
                     UINT8 rx_phy, UINT16 phy_options)
{
    ...
    if ((p_msg = (tBTA_DM_API_BLE_SET_PHY *)GKI_getbuf(sizeof(tBTA_DM_API_BLE_SET_PHY)))
                  != NULL)
    {
        bdcpy(p_msg->remote_bda, remote_device);
...
void BTA_DmBleSetDefaultPhy(UINT8 all_phy, UINT8 tx_phy, UINT8 rx_phy)
{
    ...
    if ((p_msg = (tBTA_DM_API_BLE_SET_DEFAULT_PHY *)GKI_getbuf(sizeof(tBTA_DM_API_BLE_SET_DEFAULT_PHY)))
                  != NULL)
    {
        p_msg->all_phy = all_phy;
        p_msg->tx_phy = tx_phy;
