...
static void gatt_send_conn_cback(tGATT_TCB *p_tcb)
{
    ...
    UINT16              conn_id;
    tBT_UUID gap_uuid = {LEN_UUID_128,{0}};
    tBT_UUID gatt_uuid = {LEN_UUID_128,{0}};
    tBT_UUID gattc_uuid = {LEN_UUID_128,{0}};
    tBT_UUID hid_le_uuid = {LEN_UUID_128,{0}};
    memset (&gatt_uuid.uu.uuid128, 0x81, LEN_UUID_128);
    memset (&gap_uuid.uu.uuid128, 0x82, LEN_UUID_128);
    memset (&hid_le_uuid.uu.uuid128, 0x85, LEN_UUID_128);
    memset (&gattc_uuid.uu.uuid128, 0x87, LEN_UUID_128);
    ...
    for (i = 0,  p_reg = gatt_cb.cl_rcb ; i < GATT_MAX_APPS; i++, p_reg++)
    {
        if (p_reg->in_use)
        {
            ...
            {
                conn_id = GATT_CREATE_CONN_ID(p_tcb->tcb_idx, p_reg->gatt_if);
                /*Server Code : Adding gatt_if in app_hold_link[] except for GAP_UUID , gatt_uuid
                GATTC_UUID and HID_UUID. gatt_if for these UUIDs is getting generated by default
                when the connection is made from client. These generic UUIDs if added to
                app_hold_link[] was hindering disconnect from server.*/
                if (memcmp(p_reg->app_uuid128.uu.uuid128, gap_uuid.uu.uuid128, LEN_UUID_128) &&
                    memcmp(p_reg->app_uuid128.uu.uuid128, gatt_uuid.uu.uuid128, LEN_UUID_128) &&
                    memcmp(p_reg->app_uuid128.uu.uuid128, gattc_uuid.uu.uuid128, LEN_UUID_128) &&
                    memcmp(p_reg->app_uuid128.uu.uuid128, hid_le_uuid.uu.uuid128, LEN_UUID_128))
                {
                    GATT_TRACE_DEBUG("updating app use link flag for gatt_if = %d", p_reg->gatt_if);
                    gatt_update_app_use_link_flag(p_reg->gatt_if, p_tcb, TRUE, TRUE);
                }
