...
void bta_gattc_build_skt_notification_data(RawAddress bda, Uuid char_uuid,
                                               tGATT_CL_COMPLETE* p_data,
                                               uint8_t* p) {
  size_t data_hdr_len = 6;
  uint8_t bd_addr_type = 0x01;
  uint8_t char_uuid_type = 0x02;
  uint8_t notif_data_type = 0x03;
  ...
  //LTV for bd addr
  UINT8_TO_STREAM(pp, (BD_ADDR_LEN+1));
  UINT8_TO_STREAM(pp, bd_addr_type);
  ...
  for(uint8_t k=0; k<(data_hdr_len + len_char_uuid + len_data + BD_ADDR_LEN); k++) {
    VLOG(1) << __func__ << " value: " << +p_tmp[k];
  }
}

/** write native access GATT notification data to socket */
bool bta_gattc_write_to_socket(RawAddress bda, tGATT_CL_COMPLETE* p_data, Uuid char_uuid) {
  ...
  size_t len_char_uuid = char_uuid.GetShortestRepresentationSize();
  size_t len_data = p_data->att_value.len;
  total_len = BD_ADDR_LEN + len_char_uuid + len_data + data_hdr_len;
  uint8_t p_skt_data[total_len];

  bta_gattc_build_skt_notification_data(bda, char_uuid, p_data, p_skt_data);

  //write to socket
  while (count < total_len) {
    OSI_NO_INTR(sent = send(bta_gattc_cb.gatt_skt_fd, &p_skt_data[count], total_len - count, MSG_NOSIGNAL | MSG_DONTWAIT));
    if (sent == -1) {
...
void bta_gattc_proc_native_access_notification(tBTA_GATTC_SERV* p_srcb,
  ...
  const tBTA_GATTC_CHARACTERISTIC* p_char = bta_gattc_get_characteristic_srcb(p_srcb, handle);
  bool ret = false;
  if(p_char) {
    ...
    if (it != char_uuid_list.end()) {
      ...
      ret = bta_gattc_write_to_socket(p_srcb->server_bda, p_data, p_char->uuid);
      if(ret) {
