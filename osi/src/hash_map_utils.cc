...
hash_map_utils_new_from_string_params(const char* params) {
  ...
  LOG_VERBOSE("hash_map_utils: %s: source string: '%s'", __func__, str);
  ...
  if (!items) LOG_VERBOSE("hash_map_utils: %s: no items found in string\n", __func__);
...
void hash_map_utils_dump_string_keys_string_values(
  ...
  for (const auto& ptr : map) {
    LOG_INFO("hash_map_utils: key: '%s' value: '%s'\n", ptr.first.c_str(),
             ptr.second.c_str());
  }
}
