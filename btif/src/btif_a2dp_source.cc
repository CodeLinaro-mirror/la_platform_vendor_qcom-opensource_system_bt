...
static void btif_a2dp_source_setup_codec_delayed(
  ...
  if (a2dp_codec_config == nullptr) {
    LOG_ERROR("%s: Cannot stream audio: current codec is not set",
            __func__);
    return;
  }
...
uint32_t btif_a2dp_source_read_callback(const RawAddress& peer_address,
  ...
  if (bluetooth::audio::a2dp::is_hal_2_0_enabled()) {
    ...
    bytes_read = bytes_read = UIPC_Read(*a2dp_uipc, uipc_get_ch_from_address(peer_address,
            UIPC_DATA_CH), &event, p_buf, len);
  }
...
bool btif_a2dp_source_enqueue_callback(const RawAddress& peer_address, BT_HDR* p_buf,
  ...
  LOG_VERBOSE("%s:  peer_address %s, frames_n %zu", __func__,
