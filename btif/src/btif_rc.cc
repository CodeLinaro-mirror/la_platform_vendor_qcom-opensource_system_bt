...
static bt_status_t register_notification_cmd(uint8_t label, uint8_t event_id,
                                             ...
                                             btif_rc_device_cb_t* p_dev);
static bt_status_t get_element_attributes_cmd(RawAddress* bd_addr,
  ...
  if (pmeta_msg->code == AVRC_RSP_INTERIM) {
    ...
    switch (p_rsp->event_id) {
      ...
      case AVRC_EVT_AVAL_PLAYERS_CHANGE:
        ...
        HAL_CBACK(bt_rc_vendor_ctrl_callbacks, available_player_changed_cb, &rc_addr);
        break;
...
static void handle_get_num_of_items_response(tBTA_AV_META_MSG* pmeta_msg,
  ...
  HAL_CBACK(bt_rc_vendor_ctrl_callbacks, num_of_items_rsp_cb, &rc_addr,
            p_rsp->status, p_rsp->uid_counter, p_rsp->num_items);
}
...
static void handle_avk_rc_metamsg_rsp(tBTA_AV_META_MSG* pmeta_msg) {
  ...
  if ((AVRC_OP_VENDOR == pmeta_msg->p_msg->hdr.opcode) &&
    ...
    BTIF_TRACE_DEBUG("%s parse status %d pdu = 0x%02x(%d) rsp_status = %d", __func__,
                     status, avrc_response.pdu, avrc_response.pdu,
...
/***************************************************************************
 *
 * Function         get_element_attributes_cmd
 *
...
void get_attribute_cmd(uint8_t num_attr, uint32_t* p_attr_list,
  ...
  if ((BTA_AvIsBrowsingSupported() == TRUE) &&
    ...
    get_element_attributes_cmd(&(p_dev->rc_addr), num_attr, p_attr_list);
  }
}
