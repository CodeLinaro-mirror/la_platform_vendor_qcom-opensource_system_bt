...
static void btif_a2dp_sink_audio_handle_start_playing() {
  ...
  if (btif_a2dp_sink_cb.play_alarm == nullptr) {
    LOG_ERROR("%s: unable to allocate play alarm", __func__);
    return;
  }
...
static void btif_a2dp_sink_decoder_update_event(
  ...
  APPL_TRACE_DEBUG("%s: sample_rate %d, bits_per_sample %d, channel_count %d, channel_type %d, audio_flag %d",
                   __func__, sample_rate, bits_per_sample, channel_count, channel_type, audio_flag);
  btif_a2dp_sink_cb.audio_track =
#ifndef OS_GENERIC
      BtifAvrcpAudioTrackCreate(sample_rate, bits_per_sample, channel_count, channel_type, audio_flag);
#else
      NULL;
#endif
