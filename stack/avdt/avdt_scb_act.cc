...
void avdt_scb_hdl_pkt_no_frag(AvdtpScb* p_scb, tAVDT_SCB_EVT* p_data) {
  ...
  /* RTP header is used. If the RTP header is valid, get the media payload
   * and pass it to p_sink_data_cback for handling */
...
void avdt_scb_hdl_tc_open(AvdtpScb* p_scb, tAVDT_SCB_EVT* p_data) {
  ...
  if ((p_scb->stream_config.tsep == AVDT_TSEP_SNK) && (p_scb->curr_cfg.psc_mask & AVDT_PSC_DELAY_RPT)) {
    ...
    alarm_set(delay_rpt_alarm, (period_ms_t)(SINK_DRT_ALARM_INTERVAL),(alarm_callback_t)avdt_delay_rpt_tmr_hdlr,
              (void*)p_scb);
    AVDT_TRACE_DEBUG("%s: set alarm interval:%ldms", __func__, (period_ms_t)(SINK_DRT_ALARM_INTERVAL));
  }
#endif
