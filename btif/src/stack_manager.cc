...
static void event_init_stack(void* context) {
  ...
  LOG_INFO("bt_stack_manager: %s is initializing the stack", __func__);

  if (stack_is_initialized) {
    LOG_INFO("bt_stack_manager: %s found the stack already in initialized state",
             __func__);
  } else {
  ...
  LOG_INFO("bt_stack_manager: %s finished", __func__);
...
static void ensure_stack_is_initialized(void) {
  if (!stack_is_initialized) {
    LOG_WARN("bt_stack_manager: %s found the stack was uninitialized. Initializing now.",
             __func__);
...
static void event_start_up_stack(UNUSED_ATTR void* context) {
  if (stack_is_running) {
    LOG_INFO("bt_stack_manager: %s stack already brought up", __func__);
    return;
  }
  ...
  LOG_INFO("bt_stack_manager: %s is bringing up the stack", __func__);
  ...
  if (future_await(local_hack_future) != FUTURE_SUCCESS) {
    LOG_ERROR("bt_stack_manager: %s failed to start up the stack", __func__);
...
static void event_shut_down_stack(UNUSED_ATTR void* context) {
  if (!stack_is_running) {
    LOG_INFO("bt_stack_manager: %s stack is already brought down", __func__);
    return;
  }

  LOG_INFO("bt_stack_manager: %s is bringing down the stack", __func__);
  future_t* local_hack_future = future_new();
  hack_future = local_hack_future;
...
static void ensure_stack_is_not_running(void) {
  if (stack_is_running) {
    LOG_WARN("bt_stack_manager:%s found the stack was still running. Bringing it down now.",
             __func__);
    event_shut_down_stack(NULL);
  }
}
...
static void event_clean_up_stack(void* context) {
  ...
  if (!stack_is_initialized) {
    LOG_INFO("bt_stack_manager: %s found the stack already in a clean state", __func__);
    goto cleanup;
  }

  ensure_stack_is_not_running();

  LOG_INFO("bt_stack_manager: %s is cleaning up the stack", __func__);
...
static void ensure_manager_initialized(void) {
  ...
  if (!management_thread) {
    LOG_ERROR("bt_stack_manager: %s unable to create stack management thread", __func__);
    return;
  }
}
