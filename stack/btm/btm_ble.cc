...
/* Number of BIS channels in the DBIG. Set via BTM_BleSetNumBises() before
 * creating/syncing to DBIG. 3 = Coded PHY, 4 = LE 2M PHY (default). */
static uint8_t g_dbig_num_bis = 4;
...
void BTM_BleSetNumBises(uint8_t num_bis) {
  BTM_TRACE_API("%s: num_bis=%d", __func__, num_bis);
  g_dbig_num_bis = num_bis;
}

void btm_ble_vs_dbig_status_event_internal_handler(uint8_t length, uint8_t* p_stream) {
  /* Minimum: 1 (handle) + 2 (status) + 5 (counter) + 2 (dev_id) + 10 (name) = 20 bytes.
   * Additional: BIS[n]_DevID (2*n bytes) + Broadcast_Features (2 bytes).
   * n is NOT in the event; it is set via BTM_BleSetNumBises() before DBIG creation/sync. */
  if (length < 20) {
    LOG(ERROR) << __func__ << ": Invalid DBIG status event length: " << length << " (expected >= 20)";
    return;
  }
  ...
  uint16_t dev_id;
  uint8_t name[10];
  uint16_t bis_dev_ids[MAX_BIS_COUNT] = {0};
  ...
  STREAM_TO_UINT16(dev_id, p_stream);
  STREAM_TO_ARRAY(name, p_stream, 10);

  /* Parse BIS[n]_DevID: n is NOT received in the event.
   * Use g_dbig_num_bis set by BTM_BleSetNumBises() (3=Coded PHY, 4=LE 2M). */
  uint8_t bytes_consumed = 20;  /* 1 + 2 + 5 + 2 + 10 */
  for (int i = 0; i < g_dbig_num_bis && i < MAX_BIS_COUNT; i++) {
    if ((bytes_consumed + 2) > length) break;
    STREAM_TO_UINT16(bis_dev_ids[i], p_stream);
    bytes_consumed += 2;
  }

  /* Parse Broadcast_Features: 2 octets, present after BIS[n]_DevID */
  if ((bytes_consumed + 2) <= length) {
    STREAM_TO_UINT16(broadcast_features, p_stream);
  }
  ...
  LOG(INFO) << __func__
            ...
            << ", DevID=" << loghex(dev_id)
            << ", num_bis=" << (int)g_dbig_num_bis
            << ", Broadcast_Features=" << loghex(broadcast_features);

  for (int i = 0; i < g_dbig_num_bis && i < MAX_BIS_COUNT; i++) {
    LOG(INFO) << __func__ << ": BIS[" << i << "]_DevID=" << loghex(bis_dev_ids[i]);
  }
...
uint8_t BTM_BleSetDbigParameters(tBTM_BLE_SET_DBIG_PARAMETERS_PARAM* p_data) {
  ...
  btsnd_hcic_ble_set_dbig_parameters(
      ...
      p_data->join_timeout,
      p_data->exit_timeout,
      p_data->remove_timeout,
      p_data->terminate_timeout,
      p_data->tx_power,
...
void btm_ble_vs_le_texit_dbig_event_handler(uint8_t *param, uint16_t param_len) {
  uint8_t dbig_handle;
  uint8_t reason;
  uint8_t status;
  BTM_TRACE_API("%s: param_len = %d", __func__, param_len);
  ...
  BTM_TRACE_API("%s: Received HCI_VS_LE_TExitDbIg_Complete event for DBIG_Handle=0x%02x, Reason=0x%02x, Status=0x%02x",
                __func__, dbig_handle, reason, status);

  if (hci_cmd_cmpl.texit_dbig_cmpl_cb) {
    (*hci_cmd_cmpl.texit_dbig_cmpl_cb)(status, HCI_VS_LE_TEXIT_DBIG_SUB_OPCODE);
    hci_cmd_cmpl.texit_dbig_cmpl_cb = nullptr;
  }
}

uint8_t BTM_BleTExitDbIg(tBTM_BLE_TEXIT_DBIG_PARAM* p_data) {
  BTM_TRACE_API("%s", __func__);
  ...
  hci_cmd_cmpl.texit_dbig_cmpl_cb = p_data->p_cb;
  btsnd_hcic_ble_texit_dbig(p_data->dbig_handle,
                             p_data->texit_mode,
                             p_data->reason,
                             base::Bind(&doNothing));
  return HCI_SUCCESS;
...
void btm_ble_join_control_event_handler(uint8_t *param, uint16_t param_len) {
  uint8_t dbig_handle = 0;
  uint8_t status = 0;
  BTM_TRACE_API("%s: param_len = %d", __func__, param_len);
  if (param_len < 2) {
  ...
  BTM_TRACE_API("%s: dbig_handle=0x%02x, status=0x%02x",
                __func__, dbig_handle, status);

  if (hci_cmd_cmpl.join_control_complete_cb) {
    (*hci_cmd_cmpl.join_control_complete_cb)(status, dbig_handle);
    hci_cmd_cmpl.join_control_complete_cb = nullptr;
  }
}

uint8_t BTM_BleJoinControl(tBTM_BLE_JOIN_CONTROL_PARAM* p_data) {
  BTM_TRACE_API("%s", __func__);
  ...
  /* The controller will NOT send a Command Complete event for this command.
   * The actual result arrives as a VS_Meta_Evt (btm_ble_join_control_event_handler).
   * Pass doNothing so the HCI layer's pending-command queue is released when
   * the Command Status event arrives, preventing a watchdog timeout. */
  hci_cmd_cmpl.join_control_complete_cb = p_data->p_cb;

  btsnd_hcic_ble_join_control(p_data->dbig_handle,
                               p_data->mode,
                               base::Bind(doNothing));
...
void btm_ble_set_devid_cmd_cmpl(uint8_t *param, uint16_t param_len) {
  uint8_t status;
  uint8_t sub_opcode = 0;
  BTM_TRACE_API("%s: param_len = %d", __func__, param_len);
  ...
  BTM_TRACE_API("%s: status=0x%02x, sub_opcode=0x%02x",
                __func__, status, sub_opcode);

  if (hci_cmd_cmpl.set_devid_cmpl_cb) {
    (*hci_cmd_cmpl.set_devid_cmpl_cb)(status, sub_opcode, 0);
  }
}
...
uint8_t BTM_BleSetDevId(tBTM_BLE_SET_DEVID_PARAM* p_data) {
  ...
  hci_cmd_cmpl.set_devid_cmpl_cb = p_data->p_cb;
