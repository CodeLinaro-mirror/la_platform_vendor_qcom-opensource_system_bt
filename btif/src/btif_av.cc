...
static void btif_av_check_and_start_collission_timer(int index) {
  ...
  if (interop_match_addr_or_name(INTEROP_INCREASE_COLL_DETECT_TIMEOUT, &btif_av_cb[index].peer_bda))
  {
  ...
  switch (event) {
    ...
    case BTIF_AV_CONNECT_REQ_EVT: {
        ...
        btif_iot_config_addr_set_int(btif_av_cb[index].peer_bda, IOT_CONF_KEY_A2DP_ROLE,
            (connect_req_p->uuid == UUID_SERVCLASS_AUDIO_SOURCE) ? IOT_CONF_VAL_A2DP_ROLE_SINK : IOT_CONF_VAL_A2DP_ROLE_SOURCE);
