...
static void btif_hf_upstreams_evt(UINT16 event, char* p_param)
{
    ...
    switch (event)
    {
        ...
        case BTA_AG_OPEN_EVT:
            if (p_data->open.status == BTA_AG_SUCCESS)
            {
                ...
                if (btif_hf_cb[idx].state != BTHF_CONNECTION_STATE_CONNECTING) {
                    btif_iot_config_addr_set_int(btif_hf_cb[idx].connected_bda.address, IOT_CONF_KEY_HFP_ROLE, IOT_CONF_VAL_HFP_ROLE_CLIENT);
                    btif_iot_config_addr_int_add_one(btif_hf_cb[idx].connected_bda.address, IOT_CONF_KEY_HFP_SLC_CONN_COUNT);
                }
#endif
...
#ifdef BT_IOT_LOGGING_ENABLED
                if (!ignore_rfc_fail) {
                    btif_iot_config_addr_int_add_one(btif_hf_cb[idx].connected_bda.address, IOT_CONF_KEY_HFP_SLC_CONN_FAIL_COUNT);
                }
#endif
        ...
        case BTA_AG_CONN_EVT:
            ...
            btif_iot_config_addr_set_hex(btif_hf_cb[idx].connected_bda.address, IOT_CONF_KEY_HFP_CODECTYPE,
...
#endif
            btif_iot_config_addr_set_hex(btif_hf_cb[idx].connected_bda.address, IOT_CONF_KEY_HFP_FEATURES,  p_data->conn.peer_feat, 2);
#endif
        ...
        case BTA_AG_AUDIO_CLOSE_EVT:
            ...
            if (btif_hf_cb[idx].audio_state != BTHF_AUDIO_STATE_CONNECTED) {
                btif_iot_config_addr_int_add_one(btif_hf_cb[idx].connected_bda.address, IOT_CONF_KEY_HFP_SCO_CONN_FAIL_COUNT);
            }
#endif
            btif_hf_cb[idx].audio_state = BTHF_AUDIO_STATE_DISCONNECTED;
...
#ifdef BT_IOT_LOGGING_ENABLED
        btif_iot_config_addr_set_int(btif_hf_cb[i].connected_bda.address, IOT_CONF_KEY_HFP_ROLE, IOT_CONF_VAL_HFP_ROLE_CLIENT);
        btif_iot_config_addr_int_add_one(btif_hf_cb[i].connected_bda.address, IOT_CONF_KEY_HFP_SLC_CONN_COUNT);
#endif
        return BT_STATUS_SUCCESS;
    }
...
static bt_status_t connect_audio( bt_bdaddr_t *bd_addr )
{
    ...
    if (idx != BTIF_HF_INVALID_IDX)
    {
        ...
        if (btif_hf_cb[idx].audio_state != BTHF_AUDIO_STATE_CONNECTING)
            btif_iot_config_addr_int_add_one(bd_addr->address, IOT_CONF_KEY_HFP_SCO_CONN_COUNT);
#endif
...
update_call_states:
    ...
    if (current_audio_state != BTHF_AUDIO_STATE_CONNECTING && btif_hf_cb[idx].audio_state == BTHF_AUDIO_STATE_CONNECTING)
        btif_iot_config_addr_int_add_one(btif_hf_cb[idx].connected_bda.address, IOT_CONF_KEY_HFP_SCO_CONN_COUNT);
#endif
    return status;
}
