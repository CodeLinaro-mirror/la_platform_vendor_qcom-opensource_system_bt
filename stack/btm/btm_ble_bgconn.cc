  ...
  if (p_dev_rec != NULL && p_dev_rec->device_type & BT_DEVICE_TYPE_BLE) {
    if (to_add) {
      if (p_dev_rec->ble.ble_addr_type == BLE_ADDR_PUBLIC ||
        ...
        background_connection_add(background_role, adv_handle, p_dev_rec->ble.static_addr_type,
      ...
      if (!p_dev_rec->ble.static_addr.IsEmpty() &&
        ...
        background_connection_remove(background_role, adv_handle, p_dev_rec->ble.static_addr);
        started = true;
      }
    ...
    started = true;
    if (to_add)
      background_connection_add(background_role, adv_handle, addr_type, bd_addr);
    else
      background_connection_remove(background_role, adv_handle, bd_addr);
  }
