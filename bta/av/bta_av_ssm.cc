...
enum {
  ...
  BTA_AV_OFFLOAD_REQ,
  BTA_AV_OFFLOAD_RSP,
  // Restore some data which is cleared when sep closed
  BTA_AV_RESTORE_DATA,
  BTA_AV_NUM_SACTIONS
};
...
static const uint8_t bta_av_sst_incoming[][BTA_AV_NUM_COLS] = {
    ...
    {BTA_AV_SIGNORE, BTA_AV_SIGNORE, BTA_AV_INCOMING_SST},
    /* AVDT_DISCONNECT_EVT */
    /* Go to BTA_AV_INIT_SST since AVDTP has disconnected */
...
static const uint8_t bta_av_sst_open[][BTA_AV_NUM_COLS] = {
    ...
    /* STR_CLOSE_EVT */
    /* Use BTA_AV_RESTORE_DATA to restore some data needed by sig channel,
       and then go to BTA_AV_INCOMING_SST because sig channel is still connected
     */
    {BTA_AV_STR_CLOSED, BTA_AV_RESTORE_DATA, BTA_AV_INCOMING_SST},
    /* STR_CONFIG_IND_EVT */
