...
static bool save_loc_oob_data_2_file(btif_dm_loc_oob_cb_t* oobData) {
  ...
  fwrite(oobData->oob_data.c192, 1, BT_OCTET16_LEN, fp);
  fwrite(oobData->oob_data.r192, 1, BT_OCTET16_LEN, fp);
  if (!is_empty_128bit(oobData->oob_data.c256) &&
      !is_empty_128bit(oobData->oob_data.r256)) {
      /* c256 and r256 exist, save in file*/
      fwrite(oobData->oob_data.c256, 1, BT_OCTET16_LEN, fp);
      fwrite(oobData->oob_data.r256, 1, BT_OCTET16_LEN, fp);
  }
  fclose(fp);
  return true;
}
...
static bool get_rem_oob_data_from_file(btif_dm_oob_cb_t* oobData) {
  ...
  fread(oobData->oob_data.c192, 1, BT_OCTET16_LEN, fp);
  fread(oobData->oob_data.r192, 1, BT_OCTET16_LEN, fp);
  if (fread(oobData->oob_data.c256, 1, BT_OCTET16_LEN, fp) != BT_OCTET16_LEN)
    memset(oobData->oob_data.c256, 0, BT_OCTET16_LEN);
  if (fread(oobData->oob_data.r256, 1, BT_OCTET16_LEN, fp) != BT_OCTET16_LEN)
    memset(oobData->oob_data.r256, 0, BT_OCTET16_LEN);
...
void btif_dm_proc_loc_oob(bool valid, BT_OCTET16 c192, BT_OCTET16 r192,
  ...
  memcpy(loc_oob_cb.oob_data.c192, c192, BT_OCTET16_LEN);
  memcpy(loc_oob_cb.oob_data.r192, r192, BT_OCTET16_LEN);
  memcpy(loc_oob_cb.oob_data.c256, c256, BT_OCTET16_LEN);
  memcpy(loc_oob_cb.oob_data.r256, r256, BT_OCTET16_LEN);
...
bool btif_dm_proc_rmt_oob(const RawAddress& bd_addr, BT_OCTET16 p_c192,
  ...
  memcpy(p_c192, oob_cb.oob_data.c192, BT_OCTET16_LEN);
  memcpy(p_r192, oob_cb.oob_data.r192, BT_OCTET16_LEN);
  memcpy(p_c256, oob_cb.oob_data.c256, BT_OCTET16_LEN);
  memcpy(p_r256, oob_cb.oob_data.r256, BT_OCTET16_LEN);
...
static void btif_vendor_get_link_key_event(uint16_t event, char *p_param){
  ...
  if (btif_config_get_bin(bd_addr->ToString().c_str(), "LinkKey", (uint8_t *)link_key, &size)) {
