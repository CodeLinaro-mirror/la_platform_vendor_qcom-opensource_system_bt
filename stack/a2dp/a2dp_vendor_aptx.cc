...
static bool A2DP_IsVendorCodecAptxEnabled() {
  return A2DP_GetCodecSupported(BTAV_A2DP_CODEC_INDEX_SINK_APTX);
}
...
tA2DP_STATUS A2DP_VendorIsCodecConfigMatchAptx(const uint8_t* p_codec_info) {
  ...
  a2dp_status = A2DP_ParseInfoAptx(&aptx_cie, p_codec_info, false);
...
bool A2dpCodecConfigAptxBase::setCodecConfig(const uint8_t* p_peer_codec_info,
  ...
  const tA2DP_APTX_CIE* p_a2dp_aptx_config =
      (is_source_) ? &a2dp_aptx_default_config : &a2dp_aptx_sink_default_config;

  const tA2DP_APTX_CIE* p_a2dp_aptx_cap =
      (is_source_) ? &a2dp_aptx_caps : &a2dp_aptx_sink_caps;
  ...
  if (codec_config_.sample_rate == BTAV_A2DP_CODEC_SAMPLE_RATE_NONE) {
    LOG_ERROR(LOG_TAG,
              ...
              "peer info = 0x%x",
              __func__, p_a2dp_aptx_cap->sampleRate, peer_info_cie.sampleRate);
    goto fail;
  }
  ...
  if (codec_config_.channel_mode == BTAV_A2DP_CODEC_CHANNEL_MODE_NONE) {
    LOG_ERROR(LOG_TAG,
              ...
              "peer info = 0x%x",
              __func__, p_a2dp_aptx_cap->channelMode, peer_info_cie.channelMode);
    goto fail;
  }
...
bool A2dpCodecConfigAptxSink::init() {
  ...
  if (!A2DP_VendorAuthenticateAptx()) {
    LOG_ERROR(LOG_TAG, "%s: cannot authenticate the decoder", __func__);
    return false;
  }
