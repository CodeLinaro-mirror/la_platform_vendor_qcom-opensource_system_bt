...
static void btif_iot_config_write(UINT16 event, char *p_param);
...
static future_t *init(void) {
    ...
    property_get("persist.bt.iot.enablelogging", enabled, "true");
    ...
    const char* time_str = config_get_string(config, INFO_SECTION, FILE_CREATED_TIMESTAMP, NULL);
    if (time_str != NULL) {
...
EXPORT_SYMBOL const module_t btif_iot_config_module = {
...
bool btif_iot_config_addr_get_int(const BD_ADDR peer_addr, const char *key, int *value) {
    CHECK_LOGGING_ENABLED(false);
...
bool btif_iot_config_addr_set_int(const BD_ADDR peer_addr, const char *key, int value) {
    CHECK_LOGGING_ENABLED(false);
...
bool btif_iot_config_addr_int_add_one(const BD_ADDR peer_addr, const char *key) {
    CHECK_LOGGING_ENABLED(false);
...
bool btif_iot_config_addr_get_hex(const BD_ADDR peer_addr, const char *key, int *value) {
    CHECK_LOGGING_ENABLED(false);
...
bool btif_iot_config_addr_set_hex(const BD_ADDR peer_addr, const char *key, int value, int byte_num) {
    CHECK_LOGGING_ENABLED(false);
...
bool btif_iot_config_addr_set_hex_if_greater(const BD_ADDR peer_addr,
...
bool btif_iot_config_addr_set_str(const BD_ADDR peer_addr, const char *key, const char *value) {
    CHECK_LOGGING_ENABLED(false);
...
bool btif_iot_config_addr_set_bin(const BD_ADDR peer_addr, const char *key, const uint8_t *value, size_t length) {
    CHECK_LOGGING_ENABLED(false);
...
static void btif_iot_config_write(UINT16 event, UNUSED_ATTR char *p_param) {
    CHECK_LOGGING_ENABLED((void)0);
