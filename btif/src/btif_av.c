...
static BOOLEAN btif_av_state_idle_handler(btif_sm_event_t event, void *p_data)
{
    ...
    switch (event)
    {
        ...
        case BTA_AV_RC_OPEN_EVT:
            ...
            {
                bdcpy(btif_av_cb.peer_bda.address,
...
void btif_av_trigger_suspend()
{
    ...
    btif_sm_dispatch(btif_av_cb.sm_handle, BTIF_AV_SUSPEND_STREAM_REQ_EVT, NULL);
}
...
bt_status_t btif_av_sink_execute_service(BOOLEAN b_enable)
{
    if (b_enable)
    {
        ...
        * auto-suspend av streaming on AG events(SCO or Call). The suspend shall
        * be initiated by the app/audioflinger layers */
#if defined(SDP_AVRCP_CT_BROWSE) && (SDP_AVRCP_CT_BROWSE == TRUE)
